...
if __name__ == '__main__':
	...
	updates = ['cont_logs_dir', 'heartbeat_client_dir', 'heartbeat_server_dir']
	for directory in updates:
		deleteindex = setup_config[directory].find("run")
		inputindex = setup_config[directory].find(".txt")
		if deleteindex != -1:
			deleteindex +=3
			cont_logs_filename = setup_config[directory][:deleteindex]+str(run_no)+setup_config[directory][inputindex:]
		else:
			cont_logs_filename = setup_config[directory][:inputindex]+'run'+str(run_no)+setup_config[directory][inputindex:]
		setup_config[directory] = cont_logs_filename
	...
	for i in mn_script_base.readlines():
		print i
		if i == "[CURL THING]\n":
			mn_script.write("h3 sudo bash ./hie_no_sdn_client_curl_script.sh > \""+setup_config["curl_directory"]+"\" & \n")
			...
			mn_script.write(writeThis)
		elif "[HEARTBEAT SERVER DIRECTORY]" in i:
			insertIndex = i.find("[")
			deleteIndex = i.find("]") + 1
			writeThis =  i[:insertIndex]+"\""+setup_config["heartbeat_server_dir"]+"\""+i[deleteIndex:]
			mn_script.write(writeThis)
		else:
			mn_script.write(i)		